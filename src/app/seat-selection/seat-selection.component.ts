...
export class SeatSelectionComponent implements OnInit {
  ...
  initializeSeats(): void {
    ...
    for (let i = 0; i < totalRows; i++) {
      const row = [];
      row.push({ seatNumber: `L${seatNumber++}`, isSelected: false, isAisle: false });
      row.push({ seatNumber: `L${seatNumber++}`, isSelected: false, isAisle: false });
      row.push({ seatNumber: '', isSelected: false, isAisle: true });
      row.push({ seatNumber: `R${seatNumber++}`, isSelected: false, isAisle: false });
      row.push({ seatNumber: `R${seatNumber++}`, isSelected: false, isAisle: false });
  ...
  selectSeat(seat: { seatNumber: string, isSelected: boolean, isAisle: boolean }): void {
    if (!seat.isAisle) {
